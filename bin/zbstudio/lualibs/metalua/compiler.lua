...
local bytecode_compiler = nil -- cache to avoid repeated `pcall(ZBREQUIRE(...))`
local function get_bytecode_compiler()
    if bytecode_compiler then return bytecode_compiler else
        local status, result = pcall(ZBREQUIRE, 'metalua.compiler.bytecode')
        if status then
...
function CONV :ast_to_src(...)
	ZBREQUIRE 'metalua.loader' -- ast_to_string isn't written in plain lua
	return ZBREQUIRE 'metalua.compiler.ast_to_src' (...)
end

local MT = { __index=CONV, __type='metalua.compiler' }

function M.new()
	local parser = ZBREQUIRE 'metalua.compiler.parser' .new()
