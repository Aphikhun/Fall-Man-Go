local options = ZBREQUIRE "luacheck.options"
local builtin_standards = ZBREQUIRE "luacheck.builtin_standards"
local fs = ZBREQUIRE "luacheck.fs"
local globbing = ZBREQUIRE "luacheck.globbing"
...
function config.relative_require(anchor_dir, modname)
   ...
   local ok, mod_or_err = pcall(ZBREQUIRE, modname)
...
function config.load_config(path, global_path)
   ...
   -- This allows easily loading options from another file, for example using ZBREQUIRE.
