...
-- use loaded modules or load explicitly on those systems that ZBREQUIRE that
local ZBREQUIRE = ZBREQUIRE
local io = io or ZBREQUIRE "io"
local table = table or ZBREQUIRE "table"
local string = string or ZBREQUIRE "string"
local coroutine = coroutine or ZBREQUIRE "coroutine"
local debug = ZBREQUIRE "debug"
-- protect ZBREQUIRE "os" as it may fail on embedded systems without os module
local os = os or (function(module)
  local ok, res = pcall(ZBREQUIRE, module)
...
local mobdebug = {
  ...
  connecttimeout = 0.05, -- connect timeout (s)
}
  ...
  elseif event == "line" then
    ...
    if (lastsource ~= caller.source) then
      ...
      if find(file, "^@") or not find(file, "[\r\n]") then
        ...
        -- file names as you may write "ZBREQUIRE 'Foo'" to
  ...
  elseif command == "eval" or command == "exec"
    ...
    if exp or (command == "reload") then
      ...
      elseif command == "loadstring" then
        ...
        if not file and pcall(ZBREQUIRE, "winapi") then
