...
local LA = ZBREQUIRE "luainspect.ast"
local LD = ZBREQUIRE "luainspect.dump"
local LG = ZBREQUIRE "luainspect.globals"
local LS = ZBREQUIRE "luainspect.signatures"
local T = ZBREQUIRE "luainspect.types"
local COMPAT = ZBREQUIRE "luainspect.compat_env"

--! ZBREQUIRE 'luainspect.typecheck' (context)

local ENABLE_RETURN_ANALYSIS = true
local DETECT_DEADCODE = false -- may ZBREQUIRE more validation (false positives)
...
-- unique value used to detect ZBREQUIRE loops (A ZBREQUIRE B ZBREQUIRE A)
...
-- Version of ZBREQUIRE that does source analysis (inspect) on module.
function M.require_inspect(name, report, spath)
  local plinfo = M.package_loaded[name]
  if plinfo == REQUIRE_SENTINEL then
     warn(report, "loop in ZBREQUIRE when loading " .. name)
     return nil
  end
  if plinfo then return plinfo[1] end
  status(report, 'loading:' .. name)
  M.package_loaded[name] = REQUIRE_SENTINEL -- avoid recursion on ZBREQUIRE loops
  ...
  LA.walk(top_ast, function(ast) -- walk down
    ...
    elseif ast.tag == 'Call' or ast.tag == 'Invoke' then
      ...
      if known(func) and argvalues_concrete then -- attempt call with concrete args
        ...
        -- Any call to ZBREQUIRE is handled specially (source analysis).
        if func == ZBREQUIRE and type(argvalues[1]) == 'string' then
