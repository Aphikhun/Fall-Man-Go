...
protocol.client.subsearch = "Subsearch : %s"
if success then -- ok, server was started, we are solo
  ...
  ide.timers.idle = ide:AddTimer(wx.wxGetApp(), function()
      while true do
        ...
        elseif msg:match(protocol.client.subsearch:gsub("%%s",".+$")) then
          local subsearch = msg:match(protocol.client.subsearch:gsub("%%s","(.+)$"))
          ide.config.subsearch  = subsearch
          ide:RequestAttention()
          if ide.config.subsearch and ide:GetProject() then
            local value =ide:GetProject()..ide.config.subsearch --强行指定游戏的目录
            ide:ActivateFile(value)
          end
  ...
  if msg and msg:match(protocol.server.greeting:gsub("%%s",".+$")) then
    ...
    if username ~= wx.wxGetUserName() then
      ...
      local failed = false
       if ide.config.subsearch  then
        cln:send(protocol.client.subsearch:format(ide.config.subsearch))
      end
