...
if ide.config.staticanalyzer.luacheck then
  ...
  local function build_env()
    ...
    for _, api in pairs(ide:GetInterpreter():GetAPI()) do
      ...
      local ok, tbl = pcall(ZBREQUIRE, "api/lua/" .. api)
      if ok then
  ...
  local function init()
    ...
    package.loaded.checks = {} -- make `ZBREQUIRE 'checks'` work even without `checks` module
    rawset(_G, "checks", function() end) -- provide `checks` function

    LA = ZBREQUIRE "luainspect.ast"
    LI = ZBREQUIRE "luainspect.init"
    T = ZBREQUIRE "luainspect.types"
  end
