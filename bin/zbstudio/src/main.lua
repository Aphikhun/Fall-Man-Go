...
ZBREQUIRE = ZBREQUIRE or require
...
ide = {
  GetTime = (function(ok, socket) return ok and socket.gettime or os.clock end)(pcall(ZBREQUIRE, "socket")),
  MODPREF = "* ",
  ...
  for _, file in ipairs(files) do
    if not IsDirectory(file) then
      ...
      if not ftype or not fname then
        ...
        ide.apis[ftype][fname] = file--MergeFullPath("", file)
      end
    end
  end
  ReloadAPIs("*")
...
ide.config.styles = loadfile('cfg/tomorrow.lua')('TomorrowNightEighties')
...
do
  ...
  for index = 2, #arg do
    ...
    elseif (arg[index] == "-cwd" and index+1 <= #arg) then
      ide.cwd = arg[index+1]
    elseif (arg[index] == "-subsearch" and index+1 <= #arg) then
      ide.config.subsearch = arg[index+1]
    --showMSG("subsearch = "..tostring(ide.config.subsearch))
    elseif arg[index-1] ~= "-cfg" and arg[index-1] ~= "-cwd" and arg[index-1] ~= "-subsearch"
...
do
  ...
  if ide:GetEditorNotebook():GetPageCount() == 0 then --NewFile()
  end
end
