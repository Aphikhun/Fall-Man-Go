...
local PARSE = ZBREQUIRE 'lua_parser_loose'
local LEX = ZBREQUIRE 'lua_lexer_loose'
...
return {
  ...
  typeassigns = function(editor)
    ...
    while (line <= endline) do
      ...
      elseif not iscomment[s] then
        ...
        -- handle `ZBREQUIRE` as a special case that returns a type that matches its parameter
        -- (this is without deeper analysis on loaded files and should work most of the time)
        local req = typ and typ:match("^ZBREQUIRE%s*%(?%s*['\"](.+)['\"]%s*%)?")
        typ = req or typ
        typ = (typ and typ
          :gsub("%b()","")
          :gsub("%b{}","")
  ...
  keywords = {
    ...
    [[assert collectgarbage dofile error getfenv getmetatable ipairs load loadfile loadstring
      module next pairs pcall print rawequal rawget rawlen rawset ZBREQUIRE
