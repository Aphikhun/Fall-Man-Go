...
editor:SetText('')
editor:AddText([[
  local tweaks = ZBREQUIRE("tweaks")
  local ZBREQUIRE = tweaks.ZBREQUIRE
...
for k, v in pairs({
    ...
    ree = s == 2 and "repeat ZBREQUIRE" or "",
...
editor:SetText('')
editor:AddText('local t = ZBREQUIRE("table")\nt.')
local ac = CreateAutoCompList(editor, "t.")
ok(ac ~= nil and ac:find("concat") ~= nil,
  ("Auto-complete (strategy=%s) recognizes variables set based on `ZBREQUIRE`."):format(s))

editor:SetText('')
editor:AddText('local table = ZBREQUIRE("io")\nb = ZBREQUIRE("table")\nb.')
local ac = CreateAutoCompList(editor, "b.")
ok(ac ~= nil and ac:find("concat") ~= nil,
  ("Auto-complete (strategy=%s) recognizes variables set based on `ZBREQUIRE` even when it's re-assigned."):format(s))
