...
---销毁
---@param finish boolean 是否需要执行播放完成的相关事件
function operator:destroy(finish)
    manager:destroy(self._tweenBase, finish)
end

---销毁，但是当缓动无效时不会报错
---@param finish boolean 是否需要执行播放完成的相关事件
function operator:destroySafely(finish)
