...
function dba:sendDBHttpRequest(method, url, params, body, callback, tryTimes)
    if tryTimes >= 3 then
        if url == self.dbUrl and self.dbSecondUrl then
            self:sendDBHttpRequest(method, url, params, body, callback, 1)
        else
            -- 数据请求不成功提示
...
function dba:removeData(partitionKey, key)
    local null = cjson.encode({})
    if self.locality then
        dbHandler:setData(partitionKey, key, null)
    else
        self:setData(partitionKey, key, null)
    end
end
