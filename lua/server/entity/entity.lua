...
function Entity.EntityProp:skin(value, add, buff)
	...
	local realSkillInfo  = ActorManager:Instance():getActorDefaultBodyPart(self:data("main").actorName) or {}
	...
	if add then
		for k , v in pairs(value) do 
			self.buffSkin[k] = self.buffSkin[k] or {}
			if #self.buffSkin[k] < 1 then 
				if realSkillInfo[k] then 
					table.insert(self.buffSkin[k],{name = "" , skin = realSkillInfo[k]})
				else 
					table.insert(self.buffSkin[k],{name = "", skin = ""})
				end
			end
			table.insert(self.buffSkin[k],{buffCfg = buff.cfg, skin = v})
		end 
		...
		for k , v in pairs(value) do
			...
			local skinName = nil
			local i
			for i = 1 , #self.buffSkin[k] do 
				if self.buffSkin[k][i].buffCfg == buff.cfg then
					table.remove(self.buffSkin[k] , i)
					skinName = self.buffSkin[k][#self.buffSkin[k]] and self.buffSkin[k][#self.buffSkin[k]].skin or nil
					break
				end
			end
			reset[k] = skinName 
...
function EntityServer:getInitPos()
	return (self:getTeam() and self:getTeam().initPos) or (self.map and self.map.cfg.initPos) or World.cfg.initPos
end
