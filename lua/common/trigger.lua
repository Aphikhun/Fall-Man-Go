...
local TriggerExecConfig = 
{
	...
	[trigger_exec_type.NORMAL] = {execType = TRIGGER_EXEC_TYPE.SERVER,isSingle = false, ignoreDoTrigger = true,isReload = true},
	[trigger_exec_type.NORMAL_CLIENT] = {execType = TRIGGER_EXEC_TYPE.CLIENT, isSingle = false, ignoreDoTrigger = true, isReload = true},
...
function Trigger.CheckIsLoadTrigger()
	if IS_EDITOR then --编辑器不读取蓝图
		return false 
	end
	--if World.isClient then	-- 目前客户端没有Trggier需求
	--	return false
	--end
	return true
end
...
function Trigger.LoadTriggersCommon(cfg,pathList,isDir,triggerExecType)
	...
	if not Trigger.CheckIsLoadTrigger() then return false end
...
function Trigger.LoadTriggers(cfg)			-- 兼容旧的bts继承方式
	if not Trigger.CheckIsLoadTrigger() then 
		return 
	end
	...
	if cfg.worldIndex then
		World.cfgSet[cfg.worldIndex] = nil
	end
...
function Trigger.addTrigger(cfg, triggerExecType,triggerType, trigger)
	...
	if isSingle and  cfg.triggerSet and cfg.triggerSet[triggerExecType] and cfg.triggerSet[triggerExecType][triggerType] and next(cfg.triggerSet[triggerExecType][triggerType]) then 
		...
		table.insert(TriggerList[triggerExecType],trigger)
