...
local function printScene(scene)
	p(string.format("           [RuntimeId]\t[class]                        [ParentId]"))
	p(string.format("  %20d\t%-20s", scene:getRoot():getRuntimeID(), scene:getRoot().className))
	for _, curPart in pairs(scene:getRoot():getDescendants()) do
		if not next(curPart.typeDef) then
			goto continue
		end
		local parent = curPart:getParent()
		local parentID = parent:getRuntimeID()
		p(string.format("  %20d\t%-20s %20d", curPart:getRuntimeID(), curPart.className, parentID))
		::continue::
	end
end
...
function cmds:scene(value)
	...
	if value then
		...
		p(string.format("           [scene id]\t[class]                        [ParentId]"))
	...
	if serverInfo.isREPLMode then
		...
		if func then
			if not World.isClient and World.isClient~=nil and World.serverType~="gamecenter" then
				Me = M.serverPlayerPorts[M.port] or {}
			end
