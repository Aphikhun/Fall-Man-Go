...
local function createScene(self, sceneCfg, isOnlyCreateDisNeedSync)
	...
	for _, cfg in ipairs(sceneCfg) do
		cfg.scene = scene
		-- needSync属性只能在workSpace下才生效，等网络同步重构完再考虑作为子节点也能让这个属性生效
		if cfg.properties.needSync == "false" or not isOnlyCreateDisNeedSync then
...
function Map:createSceneRegion()
    for _, info in pairs(self.cfg.scene or {}) do
