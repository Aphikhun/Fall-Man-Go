...
require "common.util.lua_hook_profiler"
...
function Lib.emitEvent(name, ...)
	local calls = eventCalls[name]
	if not calls then
		DataCacheContainer.pushDataCache(name, ...)
		return
	end
	emitCallEvent(name, ...)
end
...
function Lib.mkPath(path)
	...
	for name in string.gmatch(path, "[^/]+") do
		if not curr_path then
			...
			curr_path = string.format("%s/%s", curr_path, name)
		end
...
function Lib.deserializerStrV3(str)
	...
	if type(str)~="string" then
		Lig.logWarning(" Lib.deserializerStrV3 params must be string. Now is: ", Lib.v2s(str))
		return str
	end
