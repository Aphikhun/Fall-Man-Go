...
function handles:GameInfo(packet)
    ...
    local requestServerTime = os.time()
    local clientRunningTimeTs = 0
    Me:sendPacket({ pid = "GetServerOsTime" }, function (serverTime)
        local clientTime = os.time()
        local receivedTime = math.floor((clientTime - requestServerTime - clientRunningTimeTs) / 2)
        --serverTime = serverTime + receivedTime
        local serverTs = requestServerTime + receivedTime - serverTime
        --local serverTs = clientTime - serverTime
        local oldOsTime = os.time
        os.time = function(table)
            return oldOsTime(table) - serverTs
        end
    end)
    ...
    clientRunningTimeTs = os.time() - requestServerTime
...
function handles:ShowInfoPanel(packet)
    ...
    if packet.tipID == "game.startAlready" or packet.tipID == "game.full" then
        ---进入异常被踢出游戏（游戏已开始、满人了）
        Lib.logInfo("game full or game start, reset game ...")
