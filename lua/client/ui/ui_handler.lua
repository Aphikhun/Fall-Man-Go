...
local PropertyFuncMap = {
	...
	Resistance = {get = "getInertiaRate", set = "setInertiaRate"},
	...
	--布局容器
	Space = {get = "getSpace", set = "setSpace", getTypeFunc = getVector2},
...
function UI:getWindowInstance(window, autoCreate)
	...
	if not instance and autoCreate ~= false then
		...
		base.subscribeEvent = function(self, name, func)
			local doFunc, index = Lib.subscribeEvent(name, func)
...
function UI:openWidget(widgetName, resGroup, ...)
	...
	wndIndex = wndIndex + 1
	return self:openWindow(widgetName, instanceName, resGroup, ...)
end
...
function UI:openWindow(windowName, instanceName, resGroup, ...)
	...
	local window = self:loadWindowByResGroup(windowName, resGroup)
	if not window then
	...
	window:setName(name)
	window:setLayoutFileName(windowName)
	local id = window:getID()
	...
	if resGroup == "asset" then
		instance.__windowAssetRelPath = Lib.toFileDirName(windowName)
		instance.__windowAssetLuaScripts = Lib.toFileName(windowName)
	end

	local ok, ret = pcall(UI.loadLuaScriptByGroup, UI, instance, windowName)
	if not ok then
		releaseWindowInstance(id)
	end
	assert(ok, ret)
	...
	loadTrigger(instance, windowName)

	if type(rawget(instance, "onOpen")) == "function" then
		instance:onOpen(...)
	end
	if AT.OnWndOpen then
		AT.OnWndOpen(windowName, window, true)
	end
	if resGroup ~= "_layouts_"  then
		table.insert(windowOpenParamsMap,{windowName,instanceName, resGroup, table.pack(...)})
	end
...
function UI:openSceneWindow(windowName, instanceName, args, resGroup, ...)
	...
	assert(ok, ret)
	local sceneWindow = guiMgr:createSceneWindow(name, window, args.position, args.rotation, args.width, args.height, args.isCullBack, args.objID, args.flags)
...
function UI:closeSceneWindow(name)
	local sceneWindow = guiMgr:getSceneWindow(name)
	if not sceneWindow then
		return
	end
	...
	if UI.usePool then
		if window then
			World.Timer(1, function()
				releaseWindowInstance(window:getID())
				guiMgr:destroySceneWindow(name, true)
			end)
		else
			World.Timer(1, function()
				guiMgr:destroySceneWindow(name)
			end)
		end
	else
		World.Timer(1, function()
...
function UI:openWindowOnly(windowName, instanceName, resGroup, ...)
	...
	local window = assert(self:loadWindow(windowName, group), windowName .. "----" .. name .. " not find")
	window:setName(name)
	local id = window:getID()
	assert(not windowInstanceMap[id], id)
	local instance = UI:getWindowInstance(window)
	instance.__windowName = windowName
	instance.__groupName = group
	local ok, ret = pcall(UI.loadLuaScriptByGroup, UI, instance, windowName)
	if not ok then
		releaseWindowInstance(id)
	end
