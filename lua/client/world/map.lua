...
local function initLightGlobalParam(bm_gameSettings, map_cfg)
	...
	if map_cfg.reinhard then
		bm_gameSettings:setReinhard(map_cfg.reinhard)
	end
...
local function initMainLight(bm_gameSettings, map_cfg, isEditor)
	...
	engineSceneManager:setReinhard(0)
...
function WorldClient:loadCurMap(data, pos, mapChunkData)
	loadingUiPage("loading_page", 6)	-- enumShowType.LOAD_WORLD_START
	...
	Lib.logDebug("loadCurMap map.id = ", map.id)
