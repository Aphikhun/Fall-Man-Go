...
function M:sync_reference_node_value(additional,path,index,value)
	...
	for _,vnode in ipairs(vnodes) do
		...
		if index == "Sync_Remove" then
			VN.remove(node_sync,tonumber(value),VN.CTRL_BIT.NOTIFY  |  VN.CTRL_BIT.SYNC)
		elseif index == "Sync_Move" then
			...
			VN.move(node_sync,tonumber(move_info[1]),tonumber(move_info[2]),VN.CTRL_BIT.NOTIFY  |  VN.CTRL_BIT.SYNC)
		...
		elseif index == "Sync_Insert" then
			...
			VN.assign(node_param, "data_type", "T_Int", VN.CTRL_BIT.NOTIFY  |  VN.CTRL_BIT.SYNC )
		...
		elseif index == "Sync_Delete" then
			...
			VN.remove(parent,key,VN.CTRL_BIT.NOTIFY  |  VN.CTRL_BIT.SYNC)
		else
			VN.assign(node_sync, index, value, VN.CTRL_BIT.NOTIFY  |  VN.CTRL_BIT.SYNC )
		end
	end
