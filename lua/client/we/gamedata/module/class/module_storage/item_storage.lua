...
local function remove_model_operation_child(obj)
	for _,child in ipairs(obj.children) do
		if "PartOperation" == child.class then
			remove_operation_child(child)
		elseif "Model" == child.class then
			remove_model_operation_child(child)
		end
	end
end
	...
	--引擎文件
	{
		...
		export = function(rawval, content, save, item_self)
			...
			for _, val in ipairs(item.instances) do
				if "PartOperation" == val.class then
					remove_operation_child(val)
				elseif "Model" == val.class then
					remove_model_operation_child(val)
				end
