...
local function export_game_data(self, dump, val, save)
	...
	for _, entry in ipairs(self.config) do
		...
		local dataSet
		export_info[key], dataSet = writer(self._id, data, dump, self)
		--[FolderDataSet]存储节点资源数据的MD5
		if entry.dataSetKey and dataSet then
			export_info[entry.dataSetKey] = dataSet
		end
	end
...
function M:preprocess()
	...
	for _, entry in ipairs(self.config) do
		...
		if key ~= "triggers.bts" and key ~= "triggers_client.bts" then	-- TODO: temp，bts 不能依赖引擎数据，因为填错参数就生成不了
			local content, contentMd5, dataSet = reader(self._id, nil, self)
			if content then
				local is_import = false
				local md5
				if entry.dataSetKey then
					--[FolderDataSet]判断是否节点资源文件md5
					md5 = contentMd5
					is_import = export_info[key] ~= md5 or not self.folderConfig:check_dataset_same(export_info[entry.dataSetKey], dataSet)
				else
					--判断是否导出引擎数据
	...
	if modified then
		...
		if not dirty then
			...
			export_info = export_game_data(self, true, meta:ctor(item_value), false)
		end

		--[FolderDataSet]导出节点资源数据（编辑器）
		local exportFiles = self:export_folder_editor(item_value)
		...
		self:clear_folder_editor(exportFiles)
	end
...
--[FolderDataSet]清理节点资源数据（编辑器）
function M:clear_folder_editor()
end
...
function M:save()
	...
	-- game data
	do
		export_info = export_game_data(self, true, nil, true)
	end
	...
	do
		...
		--[FolderDataSet]导出节点资源数据（编辑器）
		local exportFiles = self:export_folder_editor(val)
