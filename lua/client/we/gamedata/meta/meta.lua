...
local meta_set_class = {
	init = function(self, path, version)
		self._meta_set = {}
		self._meta_set_next = {}	-- 升级的过程中，需要动态调整 meta 结构
		self._patch = nil
	...
	meta = function(self, type)
		return self._meta_set_next[type] or self._meta_set[type]
	end,
	...
	set_next = function(self, type, meta)
		self._meta_set_next[type] = meta
	end,

	clear_next = function(self)
		self._meta_set_next = {}
	end
}
...
local function apply_patch(val, patchs, meta_set_from, meta_set_to)
	...
	for _, patch in ipairs(patchs) do
		...
		meta_set_from:set_next(patch.type, meta_set_to:meta(patch.type))
	end

	meta_set_from:clear_next()
