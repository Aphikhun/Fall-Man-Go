...
local mapping = {
	...
	["BoundingVolume"] = function(data)
		...
		if data.type == "Box" then
			local extent = {}
			extent["x"] = data.params.x
			extent["y"] = data.params.y
			extent["z"] = data.params.z
			boundingVolume["extent"] = extent
		else
			boundingVolume["radius"] = data.radius
		end
			
		if data.type == "Capsule" or data.type == "Cylinder" then
			boundingVolume["height"] = data.height
		end
		--偏移与旋转暂不开放
		boundingVolume["offset"] = {x = 0, y = 0, z = 0}
		boundingVolume["rotation"] = {x = 0, y = 0, z = 0}
