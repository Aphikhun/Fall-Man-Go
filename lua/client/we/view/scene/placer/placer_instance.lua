...
local function nodes(list)
	local function list_children(node, nodes)
		...
		for i = 1, count do
			...
			if IInstance:get(child, "position") ~= "" and child.className ~= "SceneUIClient" then
...
function M:select(list)
	...
	for _, val in ipairs(self._list) do
		local cls = val["class"]
		local beyond = "PartOperation" == cls
		local node = IWorld:create_instance(Utils.export_inst(val),beyond)
		if beyond then
			local CW = World.CurWorld
			local manager = CW:getSceneManager()
			local scene = manager:getOrCreateScene(CW.CurMap.obj)
			IInstance:set_parent(node, scene:getRoot()) 
			node:delAllChild()
		else
			if "Model" == cls then
				check_model_op(val)
