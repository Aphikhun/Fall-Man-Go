...
function M:select(name)
	...
	if not self._node then
		self._node = assert(IWorld:create_instance({ class = "Entity", config = name }))
	end
...
function M:on_mouse_release(x, y, button, is_click)
	...
	local names = Lib.splitString(self._item, "/")
	local data = Lang:text("entity_"..names[2])
	if button == Input.MOUSE_BUTTON_TYPE.BUTTON_LEFT then
		local obj = Map:new_instance(
				Meta:meta("Instance_Entity"):ctor({
					config = self._item,
					name = data,
