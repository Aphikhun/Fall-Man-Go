	...
	local function write_merge_shapes_file(instances)
		for _,val in ipairs(instances) do
			if val.class == "PartOperation" then
				...
				if key and key ~= "" then
					write_merge_shapes_file(val.children)
					local data = {}
					local part_operation, temporary = get_instance(val)
					local children, temporary_children = get_children(val.children)
					part_operation:getMergeShapesDataAsTable(children, data)

					if temporary then
						--销毁临时对象
						IWorld:remove_instance(part_operation)
					end
					for _,child in ipairs(temporary_children) do
						--销毁临时对象
