...
function M:on_mouse_move(x, y)
	...
	local nodes = IScene:pick_rect(rect, Def.SCENE_NODE_TYPE.PART)
	local set = {}
	local diff = {}
	...
	local roots = {}
	for _, node in pairs(diff) do
		local obj = Map:query_instance(node)
		assert(obj)

		local root = obj:root()
		if not root:locked() and root:enabled() then
			roots[root] = roots[root] or {}
			local class = Map:query_instance(root:node()):class()
			if class == "Folder" then
				if not obj:locked() and obj:enabled() then
					table.insert(roots[root], node)
				end
			end
		end
	end

	for root,childs in pairs(roots) do
		if root:class() == "Folder" then
			for _,node in pairs(childs) do
				receptor:attach(node,true)
			end
		else
			receptor:attach(root:node(), true)
		end
	end
...
function M:on_hover_unlock(x,y,button)
	...
	if node then
		receptor:hover_unlock(obj:node())
	end
...
function M:on_select_obj(x,y,button,is_click)
	...
	repeat
		...
		if parent and parent:class() == "Model" then
			receptor:attach(parent:node(),true)
		else
			receptor:attach(obj:node(),true)
		end
