...
function M:save()
	if GameConfig:disable_block() then
		for _, map in pairs(self._list) do
			...
			--test staticBatchNo
			if terrains and next(terrains) then
				local list = terrains[1]:node():engineSaveCallback()
				if list and next(list) then
					for _,val in ipairs(list) do
						local id = val.instanceID
						local key = val.key
						local value = val.value
						local obj = self:query_instance(id)
						if obj then
							VN.assign(obj:vnode(), key, value, VN.CTRL_BIT.NONE)
						end
					end
					local m = Module:module("map")
					if m then
						for _,item in pairs(Module:module("map"):list()) do
...
function M:add_light_child(table)
	...
	if parentId == "" then
		if cfg.lightType == "DirectionalLight" then
			...
			cfg.position.y = cfg.lightRange + 2
		end
...
function M:drop_obj_effect(table)
	...
	childObj:node():setLocalPosition({x = 0, y = 0, z = 0})
	childObj:vnode()["position"] = childObj:node():getPosition()
	childObj:vnode()["size"] = childObj:node():getSize()
