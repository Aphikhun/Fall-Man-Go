...
function M:init(node,gui_window)
	...
	--监听引擎控件被动改变
	local gui_window_instance = UI:getWindowInstance(gui_window, true)
	gui_window_instance.onSelectStateChanged = function()
		local select = gui_window:getProperty("Selected") == "true" and true or false
		if node.Selected ~= select then
			node.Selected = select
		end
	end
	gui_window_instance.onIDChanged = function()
		local select = gui_window:getProperty("Selected") == "true" and true or false
		if node.Selected ~= select then
			node.Selected = select
		end
