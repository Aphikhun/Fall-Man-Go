...
function window_class:layout_update()
	...
	VN.assign(self._node.anchor, "HorizontalAlignment", self._guiWindow:getProperty("HorizontalAlignment"), 3)
	VN.assign(self._node.anchor, "VerticalAlignment", self._guiWindow:getProperty("VerticalAlignment"), 3)
...
function window_class:set_property(window, key, value, modify, node)
	...
	elseif key == "Text" then
		window:setProperty("Text",value)
	elseif key == "Alpha" then
		window:setProperty("Alpha",value)
	...
	elseif key == "Disabled" then
		window:setProperty("Disabled",tostring(value))
	elseif key == "MousePassThroughEnabled" then
		window:setProperty("MousePassThroughEnabled",tostring(value))
	elseif key == "MarginProperty" then
		window:setProperty("MarginProperty",value)
	elseif key == "SoundTriggerRange" then
		window:setProperty("SoundTriggerRange",value)
	elseif key == "Volume" then
		window:setProperty("Volume",value)
	elseif key == "AutoTextScale" then
		window:setProperty("AutoTextScale",tostring(value))
	...
	elseif key == "GroupID" then
		window:setProperty("GroupID",value)
	...
	elseif key == "FillType" then
		window:setProperty("FillType",value)
	elseif key == "FillPosition" then
		window:setProperty("FillPosition",value)
	elseif key == "FillArea" then
		window:setProperty("FillArea",value)
	elseif key == "AntiClockwise" then
		window:setProperty("AntiClockwise",tostring(value))
	elseif key == "IsUIMask" or key == "IsDisplayUIMask" then
		window:setProperty(key,tostring(value))
	elseif key == "CurrentValue" then
		window:setProperty(key,value)
	...
	elseif key == "HorzImageFormatting" then
		window:setProperty(key,value)
	elseif key == "VertImageFormatting" then
		window:setProperty(key,value)
	elseif key == "ForceVertScrollbar" then
		window:setProperty(key,tostring(value))
	elseif key == "ForceHorzScrollbar" then
		window:setProperty(key,tostring(value))
	elseif key == "moveAble" then
		window:setProperty(key,tostring(value))
	...
	elseif key == "hInterval" then
		window:setProperty(key, value)
		if modify then
			self:layout_update()
		end
	elseif key == "vInterval" then
		window:setProperty(key, value)
		if modify then
			self:layout_update()
		end
	...
	elseif key == "BorderEnable" then
		window:setProperty(key, tostring(value))	
	elseif key == "BorderWidth" then
		window:setProperty(key, tostring(value))
	elseif key == "TextBoldWeight" then
		window:setProperty(key, value)
	elseif key == "SkillName" then
		window:setProperty(key, value)
	elseif key == "RotateX" then
		window:setProperty(key, value)
	elseif key == "RotateY" then
		window:setProperty(key, value)
	elseif key == "RotateZ" then
		window:setProperty(key, value)	
	elseif key == "PositionX" then
		window:setProperty(key, value)
	elseif key == "PositionY" then
		window:setProperty(key, value)
	elseif key == "ActorScale" then
		window:setProperty(key, value)
	elseif key == "MinAutoTextScale" then
		window:setProperty("MinAutoTextScale", value)
	...
	elseif key == "TextXPos" or key == "HorzLabelFormatting" or key == "ImageXSizeImcress" then
		window:setProperty(key, value)
	elseif key == "enable_text_offset" then
		window:setProperty(key, tostring(value))	
	elseif key == "TextXOffset" or key == "TextYOffset" then
		window:setProperty(key, value)
	elseif key == "dir" then
		window:setProperty(key, value)
		self:layout_update()
	elseif key == "TextWordBreak" then
		window:setProperty(key, tostring(value))
	elseif key == "ImageBlendMode" or key == "ImageSrcBlend" or key == "ImageDstBlend" or key == "ImageBlendOperation" then
		window:setProperty(key,value)
	elseif key == "effectXRotate"
		or key == "effectYRotate"
		or key == "effectZRotate"
		or key == "effectScale"
		or key == "effectXPosition"
		or key == "effectYPosition"
		or key == "effectPlayMode" then
		window:setProperty(key,value)
...
function window_class:connect(modify)
	...
	if self._type == "StaticImage" then
		local image = require "we.logic.ui.widgets.static_image"
		image:init(self._node,self._guiWindow)
	end

	if self._type == "Button" then
		local button = require "we.logic.ui.widgets.button"
		button:init(self._node,self._guiWindow)
	end

	if self._type == "StaticText" then
		local text = require "we.logic.ui.widgets.static_text"
		text:init(self._node,self._guiWindow)
	...
	if self._type == "Editbox" then
		local editbox = require "we.logic.ui.widgets.editbox"
		editbox:init(self._node,self._guiWindow)
	end

	if self._type == "Checkbox" then
		local checkbox = require "we.logic.ui.widgets.checkbox"
		checkbox:init(self._node,self._guiWindow)
	end

	if self._type == "RadioButton" then
		local radio_button = require "we.logic.ui.widgets.radio_button"
		radio_button:init(self._node,self._guiWindow)
	end

	if self._type == "HorizontalSlider" or  self._type == "VerticalSlider" then
		local slider = require "we.logic.ui.widgets.slider"
		slider:init(self._node,self._guiWindow)
	end

	if self._type == "ScrollableView" then
		local view = require "we.logic.ui.widgets.scrollable_view"
		view:init(self._node,self._guiWindow)
	end

	if self._type == "ProgressBar" then
		local progressbar = require "we.logic.ui.widgets.progressbar"
		progressbar:init(self._node,self._guiWindow)
	end

	if self._type == "ActorWindow" then
		local actor_window = require "we.logic.ui.widgets.actor_window"
		actor_window:init(self._node,self._guiWindow)
	end

	if self._type == "EffectWindow" then
		local effect_window = require "we.logic.ui.widgets.effect_window"
		effect_window:init(self._node,self._guiWindow)
	end
	...
	if self._node.Font then
		Lib.subscribeEvent(Event.EVENT_PC_EDITOR_DELETE_FONT, function(font_name, next_font)
			if font_name == self._node.Font then
				VN.assign(self._node, "Font", next_font, 3)
	...
	if self._type == "SliderThumb" or self._type == "VSliderThumb" then
		local static_thumb = require "we.logic.ui.widgets.slider_thumb"
		static_thumb:init(self._node,self._guiWindow)
	end

	if self._type == "GridView" or self._type == "VerticalLayoutContainer" or self._type == "HorizontalLayoutContainer" then
		local childrenSize = self._node["ChildrenSize"]
		self._guiWindow:setProperty("ChildrenSize",Converter(childrenSize,"Vector2"))
	...
	if self._type == "GridView" then
		local space = self._node["Space"]
		self._guiWindow:setProperty("Space",Converter(space,"Vector2"))
